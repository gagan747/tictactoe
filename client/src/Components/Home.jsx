...
class Home extends Component {
  ...
  pushUserToRoom = async (roomId) => {
    try {
      const response = await fetch(
        "https://online-games-tic-tac-toe.herokuapp.com/api/joinRoom",
        {
          method: "POST",
          body: JSON.stringify({
            roomId,
  ...
  handleOnlineFriends = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://online-games-tic-tac-toe.herokuapp.com/api/getFriendsInWaiting"
      );
  ...
  getUserDetails = async () => {
    try {
      const response = await fetch(
        "https://online-games-tic-tac-toe.herokuapp.com/api/home",
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.status === 200) {
        ...
        if (data.roomId) this.handleAlreadyInRoomModal(data.roomId);
         ...
         if(this.roomIdFromParams) return this.pushUserToRoom(this.roomIdFromParams) 
      } else if (response.status === 307) {
        toast.error("Login to Continue");
